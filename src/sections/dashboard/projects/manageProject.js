...
const ManageProject = ({ setAddNewProject, isAdmin, userInfo }) => {
  ...
  useEffect(() => {
    if (!isAdmin && projects && projects !== null && projects !== undefined && projects !== "" && projects.length > 0 && userInfo && userInfo !== null && userInfo !== undefined && userInfo !== "" && Object.keys(userInfo).length > 0) {
      ...
      <Box>
        ...
        <Box mt={4}>
          <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                <Card className="TeamCard" component={Paper} onClick={() => setAddNewProject(true)} >
                  <CardContent className="d-flex flex-column align-items-center p-0">
                    ...
                    <IconButton sx={{ backgroundColor: "#F8F9FA", height: "4.5rem", width: "4.5rem", }} className="mb-3">
            ...
            {isAdmin && projects && projects !== null && projects !== undefined && projects !== "" && projects.length > 0 && projects.map((project) => (
              <Grid item xs={12} sm={6} md={4}>
                <Card className="TeamCard" component={Paper} sx={{ height: "100%" }} onClick={() => isAdmin ? route.push(`/admin/dashboard/project/${project._id}`) : route.push(`/user/dashboard/project/${project._id}`)}>
                  <CardContent className="d-flex flex-column align-items-center p-0">
                    ...
                    <Box sx={{ height: "5.5rem" }} className="d-flex align-items-center">
                      <AvatarGroup
                        total={project.collaborator && project.collaborator !== null && project.collaborator !== undefined && project.collaborator !== "" && project.collaborator.length}>
                        {project.collaborator && Array.isArray(project.collaborator) && project.collaborator.map((item) => {
                          return (<Avatar key={item} alt="img" src={getEmployeeProfile(item)} />)
                        })}
                      </AvatarGroup>
                    </Box>
                    <Box>
                      <Typography align="center">{project.collaborator.map((employeeId, index) => (<React.Fragment key={employeeId}><span>{getEmployeeName(employeeId)}</span>, </React.Fragment>))}
                      </Typography>
                    </Box>
                  </CardContent>
                </Card>
            ...
            {!isAdmin && filteredProjects && filteredProjects !== null && filteredProjects !== undefined && filteredProjects !== "" && filteredProjects.length > 0 && filteredProjects.map((project) => (
              <Grid item xs={12} sm={6} md={4}>
                <Card className="TeamCard" component={Paper} sx={{ height: "100%" }} onClick={() => isAdmin ? route.push(`/admin/dashboard/project/${project._id}`) : route.push(`/user/dashboard/project/${project._id}`)}>
                  <CardContent className="d-flex flex-column align-items-center p-0">
                    <Typography variant="h3" align="center" className="fw-semibold mb-3">{project.projectName}</Typography>
